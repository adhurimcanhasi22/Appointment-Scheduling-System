...
export class MemStorage implements IStorage {
  ...
  private async seedData() {
    ...
    const haircut = await this.createService({
      name: 'Haircut & Style',
      description:
        'Professional haircut and styling with one of our expert stylists.',
      duration: 45,
      price: 7500, // $75.00
      category: 'hair',
      image:
        'https://images.unsplash.com/photo-1560066984-138dadb4c035?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&h=400',
    });
    ...
    const coloring = await this.createService({
      ...
      category: 'hair',
      image:
        'https://images.unsplash.com/photo-1562322140-8baeececf3df?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&h=400',
    });

    const manicure = await this.createService({
      name: 'Gel Manicure',
      description:
        'Long-lasting gel polish with nail care and cuticle treatment.',
      duration: 60,
      price: 4500, // $45.00
      category: 'nails',
      image:
        'https://images.unsplash.com/photo-1604654894610-df63bc536371?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&h=400',
    });
    ...
    const facial = await this.createService({
      ...
      category: 'facial',
      image:
        'https://images.unsplash.com/photo-1570172619644-dfd03ed5d881?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&h=400',
    });
    ...
    const beardTrim = await this.createService({
      ...
      category: 'hair',
      image:
        'https://images.unsplash.com/photo-1517941313333-e4d43f13db9e?ixlib=rb-1.2.1&ixid=MnwxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8&auto=format&fit=crop&w=600&h=400',
    });

    // Create staff profiles
    const emma = await this.createStaff({
      userId: (await staffUser1).id, // Await the Promise
    ...
    await this.addStaffService({
      staffId: (await emma).id,
      serviceId: (await haircut).id,
    });
    await this.addStaffService({
      staffId: (await emma).id,
      serviceId: (await coloring).id,
    });

    await this.addStaffService({
      staffId: (await alex).id,
      serviceId: (await haircut).id,
    });
    await this.addStaffService({
      staffId: (await alex).id,
      serviceId: (await beardTrim).id,
    });

    await this.addStaffService({
      staffId: (await sophie).id,
      serviceId: (await facial).id,
    });
    await this.addStaffService({
      staffId: (await sophie).id,
      serviceId: (await manicure).id,
    });
  ...
  async createUser(user: InsertUser): Promise<User> {
    ...
    const newUser: User = {
      ...
      name: user.name,
      phone: user.phone ?? null, // Handle potential undefined
      role: user.role ?? 'client', // Provide default if undefined
      profileImage: user.profileImage ?? null, // Handle potential undefined
    };
  ...
  async createService(service: InsertService): Promise<Service> {
    ...
    const newService: Service = {
      ...
      image: service.image ?? null, // Handle potential undefined
    };
  ...
  async getStaffByUserId(userId: number): Promise<Staff | undefined> {
    for (const staff of Array.from(this.staffs.values())) {
  ...
  async createStaff(staff: InsertStaff): Promise<Staff> {
    ...
    const newStaff: Staff = {
      ...
      reviewCount: staff.reviewCount ?? null, // Handle potential undefined
    };
  ...
  async removeStaffService(
    ...
    for (const [id, ss] of Array.from(this.staffServices.entries())) {
  ...
  async setStaffAvailability(
    ...
    const newAvailability: Availability = {
      ...
      isAvailable: availability.isAvailable ?? true, // Default to true if undefined
    };
  ...
  async getUnavailableDates(staffId: number): Promise<UnavailableDate[]> {
    return Array.from(this.unavailableDates.values()).filter(
      (ud) => ud.staffId === staffId
    );
  }
  ...
  async addUnavailableDate(
    ...
    const newUnavailableDate: UnavailableDate = {
      id,
      staffId: unavailableDate.staffId,
      date: unavailableDate.date,
      reason: unavailableDate.reason ?? null, // Handle potential undefined
    };
  ...
  async createAppointment(
    ...
    const newAppointment: Appointment = {
      ...
      status: appointment.status ?? 'confirmed', // Default to "confirmed" if undefined
      notes: appointment.notes ?? null, // Handle potential undefined
      createdAt: new Date(),
    };
